...
class WebTalkRecorder {
    ...
    this.recordingWindow.innerHTML = `
      ...
      <div class="webtalk-content">
        <div class="webtalk-controls">
          <button class="webtalk-record-btn" title="Hold to record">
        ...
        <div class="webtalk-transcription">
          <div class="transcription-placeholder">
            Hold the record button and speak to transcribe...
          </div>
        </div>
        <div class="webtalk-status">Ready</div>
      </div>
    `;
  ...
  setupEventListeners() {
    const recordBtn = this.recordingWindow.querySelector('.webtalk-record-btn');
    ...
    // Record button - touch events for mobile
  ...
  makeDraggable() {
    ...
    // Setup audio recording  async setupAudio() {    try {      console.log('WebTalk: Requesting microphone access...');            // Check if microphone is available      const devices = await navigator.mediaDevices.enumerateDevices();      const audioDevices = devices.filter(device => device.kind === 'audioinput');      console.log('WebTalk: Found audio devices:', audioDevices.length);            if (audioDevices.length === 0) {        throw new Error('No microphone found on this device');      }      this.stream = await navigator.mediaDevices.getUserMedia({         audio: {          echoCancellation: true,          noiseSuppression: true,          autoGainControl: true,          sampleRate: 44100        }       });            console.log('WebTalk: Audio stream obtained:', this.stream);      console.log('WebTalk: Audio tracks:', this.stream.getAudioTracks());            // Test if tracks are active      const audioTracks = this.stream.getAudioTracks();      if (audioTracks.length > 0) {        console.log('WebTalk: Audio track state:', audioTracks[0].readyState);        console.log('WebTalk: Audio track enabled:', audioTracks[0].enabled);        console.log('WebTalk: Audio track settings:', audioTracks[0].getSettings());      }            console.log('WebTalk: Audio setup complete');      this.updateStatus('Ready to record - Microphone access granted');          } catch (error) {      console.error('WebTalk: Audio setup failed:', error);      if (error.name === 'NotAllowedError') {        this.showError('Microphone access denied. Please allow microphone access in your browser settings.');      } else if (error.name === 'NotFoundError') {        this.showError('No microphone found. Please check your audio devices.');      } else {        this.showError(`Audio setup failed: ${error.message}`);      }    }  }  // Start recording  startRecording() {    if (this.isRecording || !this.stream) {      console.log('WebTalk: Cannot start recording - already recording or no stream');      console.log('WebTalk: isRecording:', this.isRecording, 'hasStream:', !!this.stream);      return;    }    try {      console.log('WebTalk: Starting recording...');      console.log('WebTalk: Stream active:', this.stream.active);      console.log('WebTalk: Audio tracks active:', this.stream.getAudioTracks().map(t => ({        id: t.id,        enabled: t.enabled,        readyState: t.readyState,        muted: t.muted      })));      this.audioChunks = [];      this.recordingStartTime = Date.now();            // Test MediaRecorder support      console.log('WebTalk: MediaRecorder.isTypeSupported("audio/webm;codecs=opus"):',                   MediaRecorder.isTypeSupported('audio/webm;codecs=opus'));            // Create MediaRecorder      this.mediaRecorder = new MediaRecorder(this.stream, {        mimeType: 'audio/webm;codecs=opus'      });            console.log('WebTalk: MediaRecorder created, state:', this.mediaRecorder.state);      console.log('WebTalk: MediaRecorder mimeType:', this.mediaRecorder.mimeType);      this.mediaRecorder.ondataavailable = (event) => {        console.log('WebTalk: ondataavailable fired, data size:', event.data.size);        if (event.data.size > 0) {          console.log('WebTalk: Adding audio chunk, size:', event.data.size);          this.audioChunks.push(event.data);        } else {          console.warn('WebTalk: Received empty audio chunk');        }      };      this.mediaRecorder.onstop = () => {        console.log('WebTalk: MediaRecorder stopped, total chunks:', this.audioChunks.length);        this.processRecording();      };      this.mediaRecorder.onstart = () => {        console.log('WebTalk: MediaRecorder started successfully');      };      this.mediaRecorder.onerror = (event) => {        console.error('WebTalk: MediaRecorder error:', event.error);        this.showError(`Recording error: ${event.error.message}`);      };      // Start recording with smaller chunks for better capture      this.mediaRecorder.start(250); // Collect data every 250ms      console.log('WebTalk: MediaRecorder.start() called');            this.isRecording = true;      this.updateUI('recording');      this.updateStatus('🔴 Recording... (Hold button for at least 1 second)');          } catch (error) {      console.error('WebTalk: Recording start failed:', error);      this.showError(`Failed to start recording: ${error.message}`);    }  }
  ...
  async processRecording() {
    ...
    try {
      ...
      console.log('WebTalk: Sending audio for transcription...');
      ...
      try {
        ...
        if (response && response.success) {
          ...
          this.showError('Communication error with background script. This may be due to extension conflicts.');
        }
        ...
        } else if (backgroundError.message.includes('conflict')) {
          this.showError('Extension conflict detected. Try temporarily disabling the XTTS-Read-Aloud extension.');
        } else {
          this.showError(`Communication error: ${backgroundError.message}`);
        }
      }
  ...
  updateUI(state) {
    const recordBtn = this.recordingWindow.querySelector('.webtalk-record-btn');
    const recordText = this.recordingWindow.querySelector('.record-text');

    recordBtn.className = `webtalk-record-btn ${state}`;
  ...
  showErrorDialog(message) {
    alert(`WebTalk Error: ${message}\n\nPlease make sure:\n1. You've run install.bat\n2. You've run run.bat\n3. The server is running on http://localhost:9090\n4. Try temporarily disabling the XTTS-Read-Aloud extension if you have it installed`);
  }
  ...
  async sendToBackground(action, data) {
    return new Promise((resolve, reject) => {
      try {
        chrome.runtime.sendMessage(
          ...
          (response) => {
            ...
            if (response === undefined) {
              ...
              reject(new Error('Background script did not respond. This may be due to extension conflicts.'));
              return;
            }
        ...
        // Add timeout in case background script never responds
        setTimeout(() => {
          reject(new Error('Background script response timeout. Check for extension conflicts.'));
