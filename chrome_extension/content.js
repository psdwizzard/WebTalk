...
class WebTalkRecorder {
    constructor() {    this.isRecording = false;    this.mediaRecorder = null;    this.audioChunks = [];    this.recordingWindow = null;    this.stream = null;    this.outsideClickHandler = null;    this.keyHandler = null;    this.recordingStartTime = null;  }
  ...
  async setupAudio() {
    ...
    // Start recording  startRecording() {    if (this.isRecording || !this.stream) {      console.log('WebTalk: Cannot start recording - already recording or no stream');      return;    }    try {      console.log('WebTalk: Starting recording...');      this.audioChunks = [];      this.recordingStartTime = Date.now();            // Create MediaRecorder      this.mediaRecorder = new MediaRecorder(this.stream, {        mimeType: 'audio/webm;codecs=opus'      });      this.mediaRecorder.ondataavailable = (event) => {        if (event.data.size > 0) {          console.log('WebTalk: Audio chunk received, size:', event.data.size);          this.audioChunks.push(event.data);        }      };      this.mediaRecorder.onstop = () => {        console.log('WebTalk: MediaRecorder stopped');        this.processRecording();      };      // Start recording with smaller chunks for better capture      this.mediaRecorder.start(250); // Collect data every 250ms      this.isRecording = true;      this.updateUI('recording');      this.updateStatus('🔴 Recording... (Hold button for at least 1 second)');          } catch (error) {      console.error('WebTalk: Recording start failed:', error);      this.showError(`Failed to start recording: ${error.message}`);    }  }
  ...
  stopRecording() {
    ...
    // Process the recorded audio  async processRecording() {    if (this.audioChunks.length === 0) {      console.log('WebTalk: No audio chunks recorded');      this.updateStatus('No audio recorded - try holding the button longer');      this.updateUI('ready');      return;    }    // Check minimum recording duration (at least 1 second)    const recordingDuration = this.recordingStartTime ? Date.now() - this.recordingStartTime : 0;    if (recordingDuration < 1000) {      console.log('WebTalk: Recording too short:', recordingDuration, 'ms');      this.updateStatus('Recording too short - hold button for at least 1 second');      this.updateUI('ready');      return;    }
